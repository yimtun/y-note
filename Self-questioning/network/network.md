...
广播域       网络中能收到任何一台设备发出的广播包的节点集合，
当发生冲突时 冲突碎片能影响到设备的集合。

HUB一层设备，既不能分割冲突域，也不能分割广播域     HUB 所有端口都在同一个广播域内，HUB 所有端口都在同一个广播域冲突域内。

SW二层设备，能分割冲突，不能分割广播域（每个端口都是一个冲突域）

路由器三层设备，既能分割广播，又能分割冲突  每个路由器的接口都是一个广播域 每个路由器的接口都是一个冲突域
```
